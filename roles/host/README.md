...
`storage.conf` has no drop-in mechanism, so supplying settings replaces
the whole distribution file; the pre-role original is saved to
`storage.conf.ansible-orig` before the first overwrite. Setting this
variable back to an empty mapping (or uninstalling the role) restores
that original, or removes the role-written file when no original existed
(Podman then falls back to its built-in defaults, which match the role's
base values).
...
The pre-role state is saved to `policy.json.ansible-orig` before the
first overwrite (a zero-length file records that no pre-role
`policy.json` existed). Setting this variable back to an empty mapping
(or uninstalling the role) restores exactly that state - the saved
original, or no file at all - so no stale role-written trust policy
stays in effect.
...
Declaring a path adopts it: a pre-existing equivalence rule
(identical or conflicting) becomes role-owned and a conflicting
substitute is reconciled to `/var/lib/containers`. Removing a path
from this list deletes the rule and relabels the tree back to the
default policy; a pre-existing substitute is not restored.

Every accepted path is relabeled recursively (`restorecon -F`), so
only dedicated subtrees are accepted: the filesystem root,
top-level system trees (`/etc`, `/usr`, `/var`, ...), anything
below `/var/lib/containers` (already labeled), non-normalized
paths, duplicates and nested declarations are rejected.
