...
by systems other than Ansible. It applies to the CURRENTLY declared
service only: removing a unit or changing its generated service name
(e.g. via `ServiceName`) retires the previous service, `unmanaged`
does not preserve obsolete identities.
...
claims by another application's manifest are rejected, whether the
entry's `state` is `present` or `absent` (an absent declaration is
an authoritative removal request but never overrides another
application's ownership; only unowned secrets can be removed this
way). Podman
...
The credentials belong exclusively to this application: they are
written to a per-application authentication file (rootful below
`/var/lib/ansible-podman-quadlet/auth/`, rootless below the
account's `~/.config/containers/ansible-auth/`), so applications
may use different credentials for the same registry and converging
or removing one application never changes another one's logins.
Registries removed from this list are logged out of that file on
the next run; `quadlet_podman_state: "absent"` deletes the file.

The role injects `REGISTRY_AUTH_FILE` into every generated service
so startup pulls use these credentials (also after a reboot), and
`AutoUpdate=registry` containers additionally get the
`io.containers.autoupdate.authfile` label, which podman-auto-update
needs because it inspects the running container independently of
the service. Setting either of the two yourself while this list is
non-empty is rejected. For `kube` units the role cannot place that
label on the generated containers; add it in the Kubernetes YAML
when auto-updating private images, or the update falls back to the
scope defaults.

Migration note: role versions before this contract logged into the
scope-shared `auth.json`; those entries are deliberately left
untouched and can be cleaned up manually once every application
has converged with a role version using per-application files.
...
Declared directories are recorded in the application's manifest as
reference claims. Several applications MAY declare the same
directory (shared bind-mount trees are fine), but an explicit
`state: "absent"` is rejected while another application (in ANY
scope - directory paths are host-global) still declares the
directory or a descendant of it, or owns a managed file below it:
the recursive removal would take the foreign content with it.
...
managed file belongs to exactly one application - file paths are
host-global, so the claim is checked against the manifests of ALL
scopes (rootful and every rootless account), and it covers absent
entries too: `state: "absent"` is an authoritative removal request
for an unowned file but never overrides another application's
ownership. Parent directories must exist (see
`quadlet_podman_directories`).
